...
import com.keepers.conbee.common.filter.StockFilter;
...
public class FilterConfig {
	...
	public FilterRegistrationBean<LoginFilter> loginFilter(){
		...
		String[] filteringUrl = {"/revenue/*", "/stock/*", "/myPage/*"};
	...
	@Bean // 해당 메서드에서 반환된 객체를 bean으로 등록
	public FilterRegistrationBean<StockFilter> stockFilter(){
		
		// FilterRegistrationBean : 필터 등록 bean
		FilterRegistrationBean<StockFilter> filter = new FilterRegistrationBean<>();
		
		filter.setFilter(new StockFilter()); // 사용할 필터 객체 추가
		
		String[] filteringUrl = {"/stock/*"};
		
		// Arrays.asList(배열) : 배열을 List로 변환
		filter.setUrlPatterns(Arrays.asList(filteringUrl)); // 필터링할 주소 지정
		
		filter.setName("stockFilter"); // 필터 이름 지정
		
		filter.setOrder(2); // 필터 순서
