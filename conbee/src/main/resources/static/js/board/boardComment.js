...
const selectCommentList = () => {
    ...
    .then(cList => { // cList : 댓글 목록(객체 배열)
        ...
        for(let comment of cList){
            ...
            if(comment.boardCommentParent != 0)  commentRow.classList.add("child-comment");
            ...
            // 삭제되지 않은 댓글인 경우 (댓글, 버튼 생성)
                ...
                // 댓글 내용
                const boardCommentContent = document.createElement("p");
                boardCommentContent.classList.add("comment-content");
                boardCommentContent.innerHTML = comment.boardCommentContent;

                // 행에 작성자, 내용 추가
                commentRow.append(commentWriter, boardCommentContent);
                ...
                if(loginCheck){
                    ...
                    if( loginMemberNo == comment.memberNo   ){
                        ...
                        updateBtn.setAttribute("onclick", "showUpdateComment("+comment.boardCommentNo+", this)");                        
...
const boardCommentContent = document.getElementById("boardCommentContent");
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    // 2) 댓글 내용이 작성되어있나?
    if(boardCommentContent.value.trim().length == 0){ // 미작성인 경우
        alert("댓글을 작성한 후 버튼을 클릭해주세요.");

        boardCommentContent.value = ""; // 띄어쓰기, 개행문자 제거
        boardCommentContent.focus();
        return;
    }

    // 3) AJAX를 이용해서 댓글 내용 DB에 저장(INSERT)
    const data = {"boardCommentContent" : boardCommentContent.value,
    ...
    .then(result => {
        if(result > 0){ // 등록 성공
            ...
            boardCommentContent.value = ""; // 작성했던 댓글 삭제
...
function deleteComment(boardCommentNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        fetch("/comment", {
            method : "DELETE",
            headers : {"Content-type" : "application/json"},
            body : boardCommentNo // 값 하나 전달 시에는 JSON 필요 없음
        })
        .then(resp => resp.text())
        .then(result => {
            if(result > 0){
...
function showUpdateComment(boardCommentNo, btn){
                     // 댓글번호, 이벤트발생요소(수정버튼)
...
// -----------------------------------------------------------------------------------
// 댓글 수정(AJAX)
function updateComment(boardCommentNo, btn){
...
function showInsertComment(boardCommentParent, btn){
                        // 부모 댓글 번호, 클릭한 답글 버튼
    ...
    insertBtn.setAttribute("onclick", "insertChildComment("+boardCommentParent+", this)");
...
function insertChildComment(boardCommentParent, btn){
    ...
    const data = {"boardCommentContent" : boardCommentContent,
                    "memberNo" : loginMemberNo,
                    "boardNo" : boardNo,
                    "boardCommentParent" : boardCommentParent}; // JS객체
